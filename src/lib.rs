pub fn generate_applescript(_task: &str, current_dir: &str, prompt_file: &str, is_first: bool) -> String {
    // Use bash -c to properly handle command substitution
    let shell_command = format!(
        "bash -c 'cd {} && claude --dangerously-skip-permissions \"$(cat {})\" && rm {}'",
...
mod tests {
    ...
    fn test_command_structure() {
        ...
        assert!(script.contains("bash -c 'cd /work/dir && claude --dangerously-skip-permissions \"$(cat /work/dir/agent_prompt_task_1.txt)\" && rm /work/dir/agent_prompt_task_1.txt'"));
    }
}